...
Signal|Logic Voltage|Description
-|-|-
A31-0|TTL|CPU address Bus.
_BB|TTL|Bus busy.
BCLK|LVTTL*|40MHz bus clock generated by the AmigaPCI.
_BG|LVTTL|CPU bus grant.
_BR|TTL|CPU bus request.
BUFDIS|LVTTL|Disable buffers on AmigaPCI main board. When the CPU Local Bus Card is addressed, bus contention will occur if the AmigaPCI main board buffers are not disabled.
D31-0|TTL|CPU data Bus.
_INT2|TTL|Interrupt 2. Open drain.
_INT6|TTL|Interrupt 6. Open drain.
IPL0|TTL|Interrupt level 0.
IPL1|TTL|Interrupt level 1.
IPL2|TTL|Interrupt level 2.
_LOCK|TTL|CPU bus lock.
_LOCKE|TTL|CPU bus lock end.
PCLK|LVTTL|80MHz processor clock generated by the AmigaPCI.
R_W|TTL|CPU read/write.
_RESET|TTL|System reset.
_RSTOUT|TTL|Reset system request.
SIZ0|TTL|CPU transfer size 0.
SIZ1|TTL|CPU transfer size 1.
_TA|LVTTL|CPU transfer acknowledge. Open drain.
_TBI|TTL|Transfer burst inhibit. Open drain.
_TCI|TTL|CPU transfer cache inhibit. Open drain.
_TEA|LVTTL|CPU transfer error acknowledge. Open drain.
_TIP|TTL|CPU transfer in progress.
_TS|TTL|CPU transfer start.
TT0|TTL|CPU transfer type 0.
TT1|TTL|CPU transfer type 1.
UPA0|TTL|User-programmable attribute 0. Required for PCI addressing of AUTOCONFIG devices.
UPA1|TTL|User-programmable attribute 1. Required for PCI addressing of AUTOCONFIG devices.
