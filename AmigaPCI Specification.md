...
Signals are presented as bold font, such as **_FRAME** or **_TA**.  
Examples of bus data are italicized, such as *DATA0* or *ADDRESS1*.  
AmigaPCI refers to this specification or any implementation of this specification, in part or whole.
...
**NOTE:** The registers in Tables 1.3.1 and 1.3.2 relate to the Local PCI Bridge only, and do not necessarily indicate overall capability of the AmigaPCI PCI bus implementation.

Table 1.3.1. Offset $04, Command Register.
...
Table 1.3.2. Offset $06, Status Register.
Bit|Description|Supported by Local PCI Bridge Device
-|-|-
...
##### 1.3.2.3 AUTOCONFIG PCI Device Register Access
...
Table 1.3.2.3. PCI Commands for AUTOCONFIG Devices.
...
#### 1.3.3 Software Configuration Mode
...
Table 1.3.3a. Address Offset of PCI Slots For IDSEL.
...
Table 1.3.3.1. Address Offsets for Command Types[[6]](#6).
...
##### 1.3.3.1.1 PCI Configuration Access
...
#### 1.5.2 PCI Device as a Bus Driver
...
### 1.6 MC68040 Driven PCI Data Cycle
...
##### 1.6.3.2 Master Terminated - Abort
...
##### 1.6.3.3 Target Terminated - Retry
...
Figure 1.6.3.3. PCI Cycle Retry.  
<img src="/DataSheets/TimingDiagrams/PCI Cycle Retry.png" height="400"></p>

##### 1.6.3.4 Target Terminated - Disconnect
...
Figure 1.6.3.4. PCI Cycle Disconnect.  
<img src="/DataSheets/TimingDiagrams/PCI Burst Cycle Disconnect.png" height="400"></p>

##### 1.6.3.5 Target Terminated - Abort

This condition can exist any time after a target device has asserted _DEVSEL and is signaled when the target device asserts _STOP and negates _DEVSEL simultaneously. This is considered an abnormal termination in that the target device will never be able to supply to requested data. When this condition exists, the Local PCI Bridge will assert _TEA. This indicates to the MC68040 that an error condition exists and the cycle cannot continue. This condition may occur for both burst and normal cycles. This condition is treated the same as the Target Terminated - Disconnect condition by the MC68040 (Section 1.6.3.5). See Figure 1.6.3.5 for example timing.

#### 1.6.4 Delayed Cycles

Add something here.

### 1.7 PCI Driven PCI Data Cycle (DMA)
...
#### 1.7.3 PCI Fast RAM DMA Normal Mode Cycles
...
**NOTE:** The example timings under section 1.7.1 assume the DMA target device is an SDRAM device with burst mode = 4 and CAS latency = 1.

##### 1.7.3.1 PCI Fast RAM DMA Normal Read Cycle
...
##### 1.7.3.2 PCI Fast RAM DMA Normal Write Cycle

1. ADD SOME TEXT DESCRIBING THE TIMING

<img src="/DataSheets/TimingDiagrams/PCI DMA Normal Write Cycle.png" height="600"></p>

#### 1.7.4 PCI Fast RAM DMA Burst Cycles

##### 1.7.4.1 PCI Fast RAM DMA Burst Read Cycle
...
##### 1.7.4.2 PCI Fast RAM DMA Burst Read Cycle With Wait

<img src="/DataSheets/TimingDiagrams/PCI DMA Burst Read Cycle With Wait.png" width="750"></p>

##### 1.7.4.3 PCI Fast RAM DMA Burst Write Cycle

1. ADD SOME TEXT DESCRIBING THE TIMING

<img src="/DataSheets/TimingDiagrams/PCI DMA Burst Write Cycle.png" height="750"></p>

##### 1.7.4.4 PCI Fast RAM DMA Burst Write Cycle

<img src="/DataSheets/TimingDiagrams/PCI DMA Burst Write Cycle With Wait.png" width="750"></p>

#### 1.7.5 PCI Chip RAM DMA Normal Mode Cycles

Using the chip RAM space for DMA is discouraged when fast RAM is available. Chip RAM DMA does not support burst mode.

##### 1.7.5.1 PCI OCS/ECS Chip RAM DMA Normal Read Cycle

##### 1.7.5.2 PCI OCS/ECS Chip RAM DMA Normal Read Cycle

##### 1.7.5.3 PCI AGA Chip RAM DMA Normal Read Cycle

NEED INPUT.

##### 1.7.5.4 PCI AGA Chip RAM DMA Normal Read Cycle

NEED INPUT.

#### 1.7.6 Master Terminated Cycle - Timeout
